...
(define reverse-order-iter
  (lambda (list)
    (reverse-order-iter2 list '())))

(define reverse-order-iter2
  (lambda (list new)
    (if (null? list)
        new
        (reverse-order-iter2 (cdr list) (cons (car list) new)))))

(trace reverse-order-iter2)
(reverse-order-iter '(1 2 3 4))
...
(define occurs?
  (lambda (arg the-list)
    (cond ([eqv? arg the-list] (eqv? arg the-list)) 
          ([pair? the-list] (or (occurs? arg (car the-list)) (occurs? arg (cdr the-list))))
          (else #f))))
